...
const ITEMS = [
  ...
  { to: "/reservas" as const, label: "Mis reservas", icon: CalendarDays, match: (p: string) => p.startsWith("/reservas") },
  { to: "/mensajes" as const, label: "Mensajes", icon: MessageCircle, match: (p: string) => p.startsWith("/mensajes") },
];

const HIDE_ON = ["/chat/", "/paseo/", "/buscando", "/confirmar/", "/completado/"];
